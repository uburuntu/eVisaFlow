...
export async function addMemberConversation(
  ...
  while (dob === undefined) {
    ...
    if (match) {
      const dobDay = parseInt(match[1], 10);
      const dobMonth = parseInt(match[2], 10);
      const dobYear = parseInt(match[3], 10);
      if (
        dobDay >= 1 &&
        dobDay <= 31 &&
        dobMonth >= 1 &&
  ...
  if (chatId !== undefined) {
    await conversation.external(async (ctx) => {
      await Promise.allSettled(cleanupIds.map((id) => ctx.api.deleteMessage(chatId, id)));
    });
  }
